...
public class StaffDaoImpl extends HibernateDaoSupport implements StaffDao {
	...
	public CrmStaff find(String loginName, String loginPwd) {
		...
		List<CrmStaff> all = this.getHibernateTemplate().find(
				"from CrmStaff c where c.loginName=? and c.loginPwd=? ",
				loginName, loginPwd);
		if (all.size() > 0) {
			return all.get(0);
		}
	...
	public CrmStaff findById(String staffId) {
		...
		List<CrmStaff> allT = this.getHibernateTemplate().find(
				" from CrmStaff  where  id=? ", staffId);
	...
	public void update(CrmStaff crmStaff) {
		...
		if (staffCode != null) {
			...
			this.getHibernateTemplate().bulkUpdate(hql, loginName, staffName,
					gender, postId, onDutyDate, staffId);
	...
	public List<String> findByName(String name) {
		// TODO Auto-generated method stub
		String hql = "select c.loginName from CrmStaff c where c.loginName=? ";
